...
public class SlidePuzzleBoard {
	...
	private int[] generateRandomPermutation(int n) {
		...
		while (true) {
			...
			if (valid_map(permutation))
				break;
		}
		return permutation;
	}

	private boolean valid_map(int[] a)
	{
	...
	private int getinvCount(int arr[])
	{
		...
		{
			for (int j = i + 1; j < board.length * board.length - 1; j++)
			{
				if (arr[i] > arr[j])
					inv_count++;
			}
